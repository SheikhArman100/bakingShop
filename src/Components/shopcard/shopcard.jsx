...
import { Card, CardBottom, CardHead, Fav, FavButton, FoodImage, UnFav } from './shopcard.styled';
...
export const Shopcard = ({image,title,price,navigateId,loading}) => {
  ...
  return (
    <>
    {loading?<LazyCard/>:
      <Card ref={ref} variants={cardVariant}
      initial="hidden"
      animate={control}>
    
        
        
        <FoodImage src={image} alt=""  />
  
        <CardHead>{title}</CardHead>
        <FavButton onClick={FavClick}>
          {fav ?<Fav/>:<UnFav/>}
